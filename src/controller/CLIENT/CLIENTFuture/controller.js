import { CLIENTFutureService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class CLIENTFutureController {
  async findLastRecord(req, res) {
    const { days } = req.params;
    try {
      const data = await CLIENTFutureService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await CLIENTFutureService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await CLIENTFutureService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await CLIENTFutureService.findLastRecord(1);
      const newObject = {
      ...
      const data = await CLIENTFutureService.create(newObject)
...
export default new CLIENTFutureController();