import { CLIENTIndexService } from "../../../service";
...
class CLIENTIndexController {
  async seedData(req, res) {
    try {
      const result = await CLIENTIndexService.seedData()
  ...
  async findLastRecord(req, res) {
    ...
    try {
      const data = await CLIENTIndexService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await CLIENTIndexService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await CLIENTIndexService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await CLIENTIndexService.findLastRecord(1);
      const newObject = {
      ...
      const data = await CLIENTIndexService.create(newObject)
...
export default new CLIENTIndexController();