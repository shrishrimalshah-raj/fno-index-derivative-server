import { CLIENTPutService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class CLIENTPutController {
  async findLastRecord(req, res) {
    const { days } = req.params;
    try {
      const data = await CLIENTPutService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await CLIENTPutService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await CLIENTPutService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await CLIENTPutService.findLastRecord(1);
      const newObject = {
      ...
      const data = await CLIENTPutService.create(newObject)
...
export default new CLIENTPutController();