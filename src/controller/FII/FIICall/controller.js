import { FIICallService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class FIICallController {
  // async seedData(req, res) {
  //   try {
  //     const result = await FIICallService.seedData()
  ...
  async findLastRecord(req, res) {
    ...
    try {
      const data = await FIICallService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await FIICallService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await FIICallService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await FIICallService.findLastRecord(1);
      const newObject = {
      ...
      const data = await FIICallService.create(newObject)
...
export default new FIICallController();