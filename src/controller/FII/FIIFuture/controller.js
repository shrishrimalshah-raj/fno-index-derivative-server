import { FIIFutureService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class FIIFutureController {
  // async seedData(req, res) {
  //   try {
  //     const result = await FIIFutureService.seedData()
  ...
  async findLastRecord(req, res) {
    ...
    try {
      const data = await FIIFutureService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await FIIFutureService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await FIIFutureService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await FIIFutureService.findLastRecord(1);
      const newObject = {
      ...
      const data = await FIIFutureService.create(newObject)
...
export default new FIIFutureController();