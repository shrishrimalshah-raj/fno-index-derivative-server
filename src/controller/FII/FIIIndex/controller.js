import { FIIIndexService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class FIIIndexController {
  async seedData(req, res) {
    try {
      const result = await FIIIndexService.seedData()
  ...
  async findLastRecord(req, res) {
    ...
    try {
      const data = await FIIIndexService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await FIIIndexService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await FIIIndexService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await FIIIndexService.findLastRecord(1);
      const newObject = {
      ...
      const data = await FIIIndexService.create(newObject)
...
export default new FIIIndexController();