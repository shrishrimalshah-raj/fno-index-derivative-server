import { PROCallService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class PROCallController {

  async findLastRecord(req, res) {
    const { days } = req.params;
    try {
      const data = await PROCallService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await PROCallService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await PROCallService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await PROCallService.findLastRecord(1);
      const newObject = {
      ...
      const data = await PROCallService.create(newObject)
...
export default new PROCallController();