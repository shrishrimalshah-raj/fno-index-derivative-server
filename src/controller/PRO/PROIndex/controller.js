import { PROIndexService } from "../../../service";
import mongoose from 'mongoose';
import moment from 'moment';

class PROIndexController {
  async seedData(req, res) {
    try {
      const result = await PROIndexService.seedData()
  ...
  async findLastRecord(req, res) {
    ...
    try {
      const data = await PROIndexService.findLastRecord(parseInt(days));
  ...
  async find(req, res) {
    try {
      const data = await PROIndexService.find()
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async findByDate(req, res) {
    ...
    try {
      const data = await PROIndexService.findByDate(conditions)
      return res.status(200).json({ message: "Data fetch successfully", data })
    } catch (error) {
      return res.status(500).json({ "error": error })
    }
  ...
  async create(req, res) {
    ...
    try {
      const lastRecord = await PROIndexService.findLastRecord(1);
      const newObject = {
      ...
      const data = await PROIndexService.create(newObject)
...
export default new PROIndexController();